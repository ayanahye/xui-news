...
def explain():
    try:
        ...
        if method == "SHAP":
            result = shap_explainer.explain(combined_text, plot_type=plot_type, min_df=min_df, top_n=top_n)
            if plot_type in ["beeswarm", "summary", "waterfall"]:
                return Response(result["visualization"], mimetype='text/html')
        elif method == "LIME":
            result = explain_with_lime(combined_text, min_df=min_df, num_features=top_n)
        else:
...
def predict():
    try:
        ...
        combined_text = data['combined_text']
        min_df = int(data.get('min_df', 5))
        predicted_class = shap_explainer.predict_class(combined_text, min_df=min_df)
